...
that I try to share and have in sync across different machines. Storing your
configurations in a repository has been common practice for a long time, but now
it's even more common to share those repositories publicly (specially your
editor configuration). However, many years ago I only dared to do it thanks to
http://drewneil.com/[Drew Neil], who made a great
http://vimcasts.org/episodes/synchronizing-plugins-with-git-submodules-and-pathogen/[
screencast on keeping in sync your Vim configuration with Git and Github]. Back
when people made their own websites to publish their own videos, instead of
relying on big companies.

Drew has been a great help and source of inspiration with his work on
http://vimcasts.org/[Vimcasts], his books, etc. Thanks to him I discovered the
work of so many legendary plugin authors, like https://github.com/tpope/[Tim
Pope] and https://github.com/kana[Kana Natsuno].

My configuration extends a bit more than my editor, though, and there are some
little things in my Git or Bash configuration that I have collected and refined
over the years. Feel free to drop a line if you don't understand their
...
Besides the configuration and the tricks in the `init.lua` file, I started
putting some things that eventually grew to a micro-plugin in its own in the
plugin directory. For example, `foldcomments.lua` tries to imitate a feature
that I've learnt to love from http://www.qt.io/ide[Qt Creator]: fold the first
comment of a file (because it typically is a copyright notice which might
contain lots of repetitive information from the same project). Look at them in
`nvim/plugin`.

In other `nvim` subdirectories there might be some personal or even common
sense settings. Or plain fixes to the lack of completeness of the file type
support (mostly from the Vim era, and which might be obsolete now that I use a
recent Neovim).
...
TODO: Modernization opportunity: a built-in plugin that allows the user to
download missing spell files. Requires newer Neovim:
https://neovim.io/doc/user/plugins/#_builtin-plugin%3a-spellfile
...
The recommended way is to use the provided Makefile. I always do:
...
=== The two files for configuring Git

My configuration for Git might seem a bit strange, with its two separate files,
where one just includes the other, but it serves two purposes. One, at a
previous job, I realized that my coworkers just blindly copied my config, and
started making commits with my name due to it. I found that quite unacceptable
as you can imagine.

Two, at work I always need to commit to repositories with a work email, but also
a personal email, as during work I am going to commit to this configuration
repository from time to time (or to my private notes repository).

The two-file strategy fixes that.

The "normal" configuration file for Git is `~/.gitconfig`. That exists in my
repository, but its only content is one line that just includes
`~/.gitconfig.extra`. I *copy* the former, and *symlink* the latter. That makes
the "normal" one local to a computer because it's an empty husk that can be
customized without those changes being committed to the `configs` repository. So
while it is the global Git configuration for the user, the changes are not
synchronized across machines, nor shared publicly, and my user/email are safe
from sloppy coworkers or people who might clone my repository.

I can still set my name and work email in that file safely if I see fit (`git
config` will write to the only file it knows about, and things will work just
fine), then customize the personal email for my configs repo, or anything
personal, repository by repository. The global file still includes the "extra"
file which is linked to the configs on the repository here, so when I need to
add, rename or tweak Git settings I do it by hand on `gitconfig.extra`. That is
not a problem for me at all.

Additionally, to prevent mistakes with the email, my workflow tends to be to
never set the email to the global configuration file. I do it per repository,
which often is not a problem at all, given that I don't even need to type the
long command, and I just complete it from the shell history.
...
* https://github.com/sharkdp/fd (now `fd-find`)
...
* git, tree
* silversearcher-ag, fd-find, ripgrep
* htop, btop (other?)
...
* https://linux-digest.com/neovim-0-11-revolutionizes-text-rendering-with-advanced-unicode-and-emoji-support
* https://gist.github.com/kennypete/d7c3eef8fc5d9bf25dbea8fac0f1fa25
* https://github.com/neovim/neovim/pull/30014
* https://github.com/neovim/neovim/discussions/32048


=== On lazy loading, use of packadd, etc

https://vi.stackexchange.com/questions/20810/how-to-load-vim8-optional-packages-in-vimrc

[quote]
In other words, the intent of having filetype plug-ins under opt is not to add
the plug-in on demand. It is to add the plug-in conditionally, based on presence
of some feature in your system which can be dynamically detected from your
.vimrc. For instance, you might want to detect whether a node binary exists or
if eslint is present or similar, and only enable vim-javascript in those cases.

https://github.com/neovim/neovim/issues/35562

[quote]
Lazy-loading is already supported via autoload/ and lua/. Plugin authors can
provide "lazy loading" by providing a plugin/<name>.lua file which defines their
commands and keymappings. This file should be small, and should not eagerly
require() the rest of your plugin. Commands and mappings should do the
require(). (P.S.: the code required to do this is THE SAME AMOUNT OF CODE needed
to configure "lazy loading" in your lazy.nvim config, so avoiding plugin/*.lua
...
Regardless, the current status quo is horrible, and some authors may not have
the will or capacity to improve their plugins' startup impact.
--
...
All of this are working examples of how to use the new options in a more
structured way instead of strings:

    lua vim.opt.listchars:append({ trail = '_'})
    lua vim.opt.listchars:remove('trail')
    lua vim.opt.listchars:remove({'trail', 'lead'})
    lua vim.opt.listchars = vim.opt.listchars + { trail = 'x'}

LuaJIT 3.0 gained some syntax extensions (and 2.1 got some backported). Those
include the "compound operators" like `+=`, so the last line could eventually be
even become simpler (if Neovim adopts such version and one can assume the risk
of incompatibility):

    lua vim.opt.listchars += { trail = 'x'}
...
* https://github.com/Prgebish/sigil.nvim Prettify math symbols (and fix movement)
* https://github.com/brianhuster/live-preview.nvim (preview AsciiDoc as you type)
* https://github.com/nvimtools/hydra.nvim (submode alternative, maintained fork)
